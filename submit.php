...
$page_title = "Submit Ordinance";
$current_page = "submit";

$message = '';
$error = '';
$is_edit = isset($_GET['id']);
$ord_id = $is_edit ? (int)$_GET['id'] : null;
$ord_data = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        check_rate_limit($pdo, 'submit', 15, 15); // Allow 15 submissions/edits per 15 minutes
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
    ...
    $ord_number = trim($_POST['ord_number']);
    $dept = trim($_POST['department']);
    $sponsor = trim($_POST['sponsor']);
    $co_authors = trim($_POST['co_authors'] ?? '');
    $date_enacted = $_POST['date_enacted'];
    $status = $_POST['status'] ?? '';
    // Keep explicit reading-stage statuses and default new submissions to 'for_reading'
    if (empty($status)) {
        $status = 'for_reading';
    }
    ...
    if (empty($title) || empty($dept) || empty($date_enacted)) {
        ...
        try {
            ...
            // Handle File Upload
            $hard_copy_path = $is_edit ? $ord_data['hard_copy_path'] : null;
            if (isset($_FILES['hard_copy']) && $_FILES['hard_copy']['error'] !== UPLOAD_ERR_NO_FILE) {
                if ($_FILES['hard_copy']['error'] !== UPLOAD_ERR_OK) {
                    $upload_errors = [
                    ...
                    $err_msg = $upload_errors[$_FILES['hard_copy']['error']] ?? "Unknown upload error.";
                    throw new Exception("File upload failed: " . $err_msg);
                }
                ...
                $new_hard_copy_path = $upload_dir . $new_file_name;
                if (!move_uploaded_file($file_tmp, $new_hard_copy_path)) {
                    throw new Exception("Failed to save the uploaded file.");
                }

                // Delete old file if it exists
                if ($is_edit && !empty($ord_data['hard_copy_path']) && file_exists($ord_data['hard_copy_path'])) {
                    @unlink($ord_data['hard_copy_path']);
                }

                $hard_copy_path = $new_hard_copy_path;
            }

            if ($is_edit) {
                $stmt = $pdo->prepare("
                    UPDATE ordinances SET 
                        ordinance_number = ?, title = ?, description = ?, 
                        date_enacted = ?, status = ?, department = ?, 
                        main_author = ?, co_authors = ?, soft_copy = ?, hard_copy_path = ?, notes = ?
                    WHERE id = ?
                ");
                $stmt->execute([$ord_number, $title, $title, $date_enacted, $status, $dept, $sponsor, $co_authors, $soft_copy, $hard_copy_path, $notes, $ord_id]);
                
                $pdo->prepare("DELETE FROM ordinance_tags WHERE ordinance_id = ?")->execute([$ord_id]);
            } else {
                if (empty($ord_number)) {
                    $ord_number = 'ORD-' . date('Y') . '-' . str_pad(rand(1, 999), 3, '0', STR_PAD_LEFT);
                }
                $stmt = $pdo->prepare("
                    INSERT INTO ordinances (ordinance_number, title, description, date_enacted, status, department, main_author, co_authors, soft_copy, hard_copy_path, notes)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ");
                $stmt->execute([$ord_number, $title, $title, $date_enacted, $status, $dept, $sponsor, $co_authors, $soft_copy, $hard_copy_path, $notes]);
            ...
            if (!empty($selected_tags)) {
                ...
                foreach ($selected_tags as $tag_id) {
                    $tag_stmt->execute([$ord_id, $tag_id]);
                }
            }

            $action = $is_edit ? 'update' : 'create';
            $log_stmt = $pdo->prepare("INSERT INTO activity_log (user_id, action, target_type, target_id, target_name, description) VALUES (?, ?, 'ordinance', ?, ?, ?)");
            $log_stmt->execute([$_SESSION['user_id'], $action, $ord_id, $ord_number, "Ordinance $ord_number " . ($is_edit ? "updated" : "created")]);

            $pdo->commit();
            $_SESSION['submit_success'] = true;
            $_SESSION['submit_action'] = $is_edit ? 'updated' : 'created';
            $_SESSION['submit_ord_id']  = $ord_id;
            $_SESSION['submit_ord_num'] = $ord_number;
            header("Location: submit.php?done=1" . ($is_edit ? "&id=$ord_id" : ""));
            exit;
...
$current_tag_ids = [];
if ($is_edit) {
    $stmt = $pdo->prepare("SELECT tag_id FROM ordinance_tags WHERE ordinance_id = ?");
    $stmt->execute([$ord_id]);
...
<div class="space-y-6 max-w-[1440px] mx-auto pb-20">
    ...
    <div class="flex items-center gap-2 text-[13px] font-bold text-on-surface-variant">
        <a href="database" class="hover:text-navy transition-colors">Submissions</a>
        <span class="text-outline-variant">+</span>
        <span class="text-navy">New Ordinance</span>
    </div>
    
    <!-- Title -->
    <div>
        <h1 class="text-[32px] font-bold text-navy leading-tight">Submit New Ordinance</h1>
        <p class="text-sm text-on-surface-variant mt-1">Complete the required metadata and document content for legislative review.</p>
    </div>
    ...
    <form action="submit.php<?php echo $is_edit ? '?id='.$ord_id : ''; ?>" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white rounded-lg border border-outline-variant shadow-sm p-8 space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-navy mb-2 uppercase tracking-tight">Ordinance Title <span class="text-error">*</span></label>
                        <input type="text" name="title" required value="<?php echo $is_edit ? htmlspecialchars($ord_data['title']) : ''; ?>"
                               ...
                               placeholder="e.g., Amendment to Zoning Code Section 4A">
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-xs font-bold text-navy mb-2 uppercase tracking-tight">Proposed Number (Optional)</label>
                            <input type="text" name="ord_number" value="<?php echo $is_edit ? htmlspecialchars($ord_data['ordinance_number']) : ''; ?>"
                                   class="w-full px-4 py-3 bg-surface-container-lowest border border-outline-variant rounded focus:outline-none focus:border-navy text-sm placeholder:text-outline"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="department" required class="w-full px-4 py-3 bg-surface-container-lowest border border-outline-variant rounded focus:outline-none focus:border-navy text-sm appearance-none">
                                    ...
                                    foreach ($depts as $d): ?>
                                        <option value="<?php echo htmlspecialchars($d); ?>" <?php echo ($is_edit && $ord_data['department'] == $d) ? 'selected' : ''; ?>><?php echo htmlspecialchars($d); ?></option>
                                    <?php endforeach; ?>
                                </select>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="sponsor" value="<?php echo $is_edit ? htmlspecialchars($ord_data['main_author']) : ''; ?>"
                                   class="w-full px-4 py-3 bg-surface-container-lowest border border-outline-variant rounded focus:outline-none focus:border-navy text-sm placeholder:text-outline"
                        ...
                        <div>
                            ...
                            <input type="text" name="co_authors" value="<?php echo $is_edit ? htmlspecialchars($ord_data['co_authors'] ?? '') : ''; ?>"
                                   class="w-full px-4 py-3 bg-surface-container-lowest border border-outline-variant rounded focus:outline-none focus:border-navy text-sm placeholder:text-outline"
                ...
                <div class="space-y-6">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="relative">
                                <input type="date" name="date_enacted" required value="<?php echo $is_edit ? $ord_data['date_enacted'] : date('Y-m-d'); ?>"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="status" class="w-full px-4 py-3 bg-surface-container-lowest border border-outline-variant rounded focus:outline-none focus:border-navy text-sm appearance-none">
                                    <option value="for_reading" <?php echo (!$is_edit || in_array($ord_data['status'], ['under_review','for_reading'])) ? 'selected' : ''; ?>>For Reading</option>
                                    <option value="under_review" <?php echo ($is_edit && $ord_data['status'] == 'under_review') ? 'selected' : ''; ?> style="display:none">(legacy) Pending Review</option>
                                    <option value="draft" <?php echo ($is_edit && $ord_data['status'] == 'draft') ? 'selected' : ''; ?>>Draft</option>
                                    <option value="active" <?php echo ($is_edit && $ord_data['status'] == 'active') ? 'selected' : ''; ?>>Active / Enacted</option>
                                    <option value="amended" <?php echo ($is_edit && $ord_data['status'] == 'amended') ? 'selected' : ''; ?>>Amended</option>
                                    <option value="repealed" <?php echo ($is_edit && $ord_data['status'] == 'repealed') ? 'selected' : ''; ?>>Repealed</option>
                                </select>
                                <span class="absolute right-4 top-1/2 -translate-y-1/2 text-outline-variant">+</span>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="4" 
                                  class="w-full px-4 py-3 bg-surface-container-lowest border border-outline-variant rounded focus:outline-none focus:border-navy text-sm placeholder:text-outline"
                                  placeholder="Add any additional notes or remarks here..."><?php echo $is_edit ? htmlspecialchars($ord_data['notes']) : ''; ?></textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-outline-variant shadow-sm p-8 space-y-8">
            ...
            <div class="space-y-4">
                <div class="space-y-1">
                    ...
                    <p class="text-xs text-on-surface-variant">Upload the scanned or finalized PDF version of the ordinance.</p>
                </div>
                ...
                <div id="pdf-drop-zone" class="relative border-2 border-dashed border-outline-variant rounded-lg p-12 bg-surface-container-lowest flex flex-col items-center justify-center text-center group hover:border-navy transition-all">
                    ...
                    <?php if ($is_edit && $ord_data['hard_copy_path']): ?>
                        <div id="pdf-current" class="mt-4 p-2 bg-green-50 text-green-700 text-[10px] font-bold rounded border border-green-100 uppercase tracking-widest">
                            Current: <?php echo basename($ord_data['hard_copy_path']); ?>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="space-y-4">
                ...
                <div class="border border-outline-variant rounded overflow-hidden">
                    ...
                    <textarea name="soft_copy" rows="18" 
                              ...
                              placeholder="Enter the official text of the ordinance here..."><?php echo $is_edit ? htmlspecialchars($ord_data['soft_copy']) : ''; ?></textarea>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fileInput = document.getElementById('hardCopyInput');
    const promptEl = document.getElementById('pdf-prompt');
    const selectedEl = document.getElementById('pdf-selected');
    const filenameEl = document.getElementById('pdf-filename');
    const currentEl = document.getElementById('pdf-current');
    const iconWrap = document.getElementById('pdf-icon-wrap');

    if (dropZone && fileInput) {
        // Prevent default drag behaviors to stop browser from opening the file
        ...
        ['dragenter', 'dragover'].forEach(eventName => {
            dropZone.addEventListener(eventName, (e) => {
                e.preventDefault();
                e.stopPropagation();
                dropZone.classList.add('border-navy', 'bg-surface-container');
        ...
        ['dragleave', 'drop'].forEach(eventName => {
            dropZone.addEventListener(eventName, (e) => {
                e.preventDefault();
                e.stopPropagation();
                dropZone.classList.remove('border-navy', 'bg-surface-container');
                dropZone.classList.add('border-outline-variant');
            }, false);
        });

        // Handle drop event explicitly just in case standard input behavior is bypassed
        dropZone.addEventListener('drop', (e) => {
            const dt = e.dataTransfer;
            const files = dt.files;
            if (files.length > 0) {
                fileInput.files = files;
                // Trigger change event manually so validation/UI updates
                fileInput.dispatchEvent(new Event('change'));
            }
        }, false);

        // Handle file selection (via browse or drag-and-drop)
        fileInput.addEventListener('change', (e) => {
            ...
            if (files.length > 0) {
                ...
                // Display newly selected file details
...
<?php
    ...
    $submit_ord_id  = $_SESSION['submit_ord_id']  ?? null;
    $submit_ord_num = $_SESSION['submit_ord_num'] ?? 'Ordinance';
    $is_edit_done   = ($submit_action === 'updated');
    unset($_SESSION['submit_success'], $_SESSION['submit_action'], $_SESSION['submit_ord_id'], $_SESSION['submit_ord_num']);
?>
<style>
    ...
    <div id="sm-card"
        ...
        <div class="space-y-1">
            ...
            <p class="text-[13px]" style="color:#6b7280;">
                Updating legislative records&hellip;
            </p>
        </div>

        <!-- Buttons -->
        <div class="w-full flex flex-col gap-2 pt-1">
            <?php if ($is_edit_done && $submit_ord_id): ?>
            <a href="submit.php?id=<?php echo (int)$submit_ord_id; ?>"
...
<script>
    function smClose(){ window.location.href='database.php'; }
