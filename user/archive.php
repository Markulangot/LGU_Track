...
<div class="space-y-6 max-w-[1440px] mx-auto pb-20">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-on-surface-variant mt-1">Cabinet view of repealed, superseded, and historical legislative records.</p>
        </div>
    ...
    <!-- Search & Filters -->
    <div class="bg-white p-4 rounded-lg border border-outline-variant shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div class="relative flex-1">
            <img src="../assets/icons/search.png" class="w-4 h-4 absolute left-4 top-1/2 -translate-y-1/2 opacity-40">
            <input type="text" id="archiveSearchInput" placeholder="Instant Search (Ordinance No, Title, Author, Year)..." 
        ...
        <div class="flex items-center gap-2">
            <button class="px-4 py-2.5 bg-surface-container-lowest border border-outline-variant rounded text-xs font-bold text-navy hover:bg-surface-container transition-all">
                Export Backup (ZIP)
            </button>
        </div>
    </div>
    ...
    <div class="space-y-8" id="archiveContainer">
        <?php if (empty($archives_by_year)): ?>
            <div class="flex flex-col items-center justify-center py-20 bg-white rounded-lg border border-outline-variant shadow-sm">
            ...
            <?php foreach ($archives_by_year as $year => $ordinances): ?>
                <div class="cabinet-year-group" data-year="<?php echo htmlspecialchars($year); ?>">
                    ...
                    <div class="flex items-center gap-4 mb-4 sticky top-0 bg-surface z-10 py-2">
                        <div class="w-10 h-10 bg-navy text-white rounded-lg flex items-center justify-center font-bold text-lg shadow-md">
                    ...
                    <!-- Grid of Files (Bento Style Cards) -->
                        ...
                        <?php foreach ($ordinances as $ord): ?>
                            <div class="archive-card bg-white p-5 rounded-lg border border-outline-variant hover:border-navy hover:shadow-md transition-all group flex flex-col relative"
                                ...
                                <div class="flex justify-between items-start mb-3">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <span class="text-[10px] text-outline font-bold uppercase tracking-widest"><?php echo date('M d', strtotime($ord['date_enacted'])); ?></span>
                                        </div>
                                    </div>
                                    <span class="status-badge px-2.5 py-0.5 rounded-sm border border-red-100 bg-red-50 text-red-700 text-[9px] font-bold uppercase tracking-widest">
                                        REPEALED
                                    </span>
                                </div>
                                
                                <h4 class="text-[14px] font-medium text-on-surface mb-2 font-doc italic leading-snug line-clamp-2 flex-1 archive-title">
                                ...
                                <div class="mt-auto pt-4 border-t border-outline-variant flex items-center justify-between">
                                    <div class="flex items-center gap-1.5 text-[11px] text-outline font-medium truncate max-w-[150px]">
                                    ...
                                    <div class="flex gap-2">
                                        <a href="view.php?id=<?php echo $ord['id']; ?>" class="w-7 h-7 rounded-full bg-surface-container hover:bg-navy flex items-center justify-center transition-all group-hover:bg-navy/10">
                                        ...
                                        <?php if ($ord['hard_copy_path']): ?>
                                            <button onclick="openPdfPreview('../<?php echo htmlspecialchars($ord['hard_copy_path']); ?>')" class="w-7 h-7 rounded-full bg-red-50 hover:bg-red-100 flex items-center justify-center transition-all">
...
<!-- PDF Preview Modal -->
<div id="pdfPreviewModal" class="fixed inset-0 z-[400] hidden flex-col" style="background:rgba(10,0,0,.82);backdrop-filter:blur(4px)">
    <div class="flex items-center justify-between px-5 py-3 flex-shrink-0" style="background:rgba(127,0,0,.95);border-bottom:1px solid rgba(255,255,255,.1)">
        ...
        <div class="flex items-center gap-3">
            <a id="pdfPreviewDownload" href="#" download class="px-4 py-1.5 text-xs font-bold text-white rounded hover:bg-white/20 transition-all" style="background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.25)">⬇ Download</a>
            <button onclick="closePdfPreview()" class="w-8 h-8 rounded flex items-center justify-center text-white text-lg hover:bg-white/20 transition-all" style="background:rgba(255,255,255,.12)" aria-label="Close">✕</button>
        </div>
    </div>
